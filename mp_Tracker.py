...
class Tracker(SLAMParameters):
    def __init__(self, slam):
        ...
        self.gt_poses = self.trajmanager.gt_poses
    ...
    def save_poses(self):
        np.save(self.dataset_path + "/posesnp",np.array(self.trajmanager.gt_poses))
        import math
        poses = self.poses
    ...
    def save_gt_poses(self):
        import math
        poses = copy.deepcopy(self.trajmanager.gt_poses)
        img_num = self.gt_poses.__len__()
        dataset = self.dataset_path
    ...
    def save_gt_poses_continus_error(self):
        import math
        poses = copy.deepcopy(self.trajmanager.gt_poses)
        img_num = self.gt_poses.__len__()
        dataset = self.dataset_path
    ...
    def save_frame_points(self,points,idx):
        ...
        if not os.path.exists(self.dataset_path + "/pcd/"):
            # 如果文件夹不存在，则创建它
            os.makedirs(self.dataset_path + "/pcd/")
        if not os.path.exists(self.dataset_path + "/pcd/" + str(idx) + ".pcd"):
            o3d.io.write_point_cloud(self.dataset_path + "/pcd/" + str(idx) + ".pcd", pcd)
